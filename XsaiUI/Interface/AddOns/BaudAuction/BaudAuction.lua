...
local EventFuncs = {
  ADDON_LOADED = function(self, AddOn)
    ...
    if not Config then
      ...
      Config.MatchBid = BaudAuctionMatchBid;
    end
    if(Config.MatchBid==nil)then
      Config.MatchBid = true;
    end
    BaudAuctionBuyoutCheck:SetChecked(Config.MatchBid);
    if(Config.LastTime==nil)then
      Config.LastTime = 3;
    end
    AuctionsRadioButton_OnClick(Config.LastTime);
    hooksecurefunc("AuctionsRadioButton_OnClick", function(Value)
      Config.LastTime = Value;
    end);
  end,
