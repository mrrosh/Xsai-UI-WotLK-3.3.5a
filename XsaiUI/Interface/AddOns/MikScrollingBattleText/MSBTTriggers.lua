...
local function CategorizeTrigger(triggerSettings)
 ...
 for mainEvent, conditionsString in string_gmatch(triggerSettings.mainEvents .. "&&", "(.-)%{(.-)%}&&") do
  ...
  -- Other events.
   ...
   -- Health/Mana/Energy/Rage/Runic Power.
   if (mainEvent == "UNIT_HEALTH" or mainEvent == "UNIT_MANA" or mainEvent == "UNIT_ENERGY" or mainEvent == "UNIT_RAGE" or mainEvent == "UNIT_RUNIC_POWER") then
...
local function OnEvent(this, event, arg1, ...)
 ...
 elseif (event == "UNIT_RUNIC_POWER") then
  HandleHealthAndPowerTriggers(arg1, event, UnitMana(arg1), UnitManaMax(arg1), SPELL_POWER_RUNIC_POWER)
